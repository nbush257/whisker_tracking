function playWhiskerPolyFitVideo(seqPath,whiskerData,manipData,framesPerSecond,xBase,yBase,startFrame,stopFrame,xTol,yTol)
% function playWhiskerPolyFitVideo(seqPath,whiskerData,manipData,framesPerSecond,xBase,yBase,startFrame,stopFrame,xTol,yTol)
%
...
for frameCount = startFrame:stopFrame
    ...
    %plot(manipData(thisFrameManipIndexes).xPolyFit, ...
    %    manipData(thisFrameManipIndexes).yPolyFit, 'w-');
    %end
    ...
    %end
        ...
        if whiskerData(thisFrameWhiskerIndexes(count)).stableBasepoint
            plot(whiskerData(thisFrameWhiskerIndexes(count)).xPolyFit, ...
                whiskerData(thisFrameWhiskerIndexes(count)).yPolyFit, ...
                'go','MarkerSize',2);
