...
for img = image_span
    ...
    hold on % stay!
    scatter(xf{img},yf{img}) % plot the already-tracked part of the whisker
    [x_to_tip{img},y_to_tip{img}] = getpts(fig);
    
    % Polyfit the results
%     xx = [xf{img};x_to_tip];
%     yy = [yf{img};y_to_tip];
    whisker_points = sortrows([[xf{img};x_to_tip{img}],[yf{img};y_to_tip{img}]],1); % sort the whisker points
    whisker_points = whisker_points(~isnan(whisker_points(:,1)),:); % remove those pesky NaNs
    new_extent = whisker_points(1,1):whisker_points(end,1); % determine new x-coords
%     splined_y = spline(whisker_points(:,1),whisker_points(:,2),new_extent); % fit spline
    pfit_x{img} = new_extent;
    pfit = polyfit(whisker_points(:,1),whisker_points(:,2),polyfit_degree);
    pfit_y{img} = polyval(pfit,new_extent);
