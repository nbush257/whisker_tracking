function [whiskerData] = flagDiscontigFrames(whiskerData,tol)
% function [whiskerData] = flagDiscontigFrames(whiskerData,tol)
%
% This function identifies frames in which the whisker length is
% substantially discontiguous compared to the length on adjacent frames.
% Discontiguous flags are flagged by the function to be removed from later
% analysis.
%
% Sudden changes in length suggest a tracking error; most commonly, they
% suggest that part of the manipulator is being included in the tracked
% whisker, rendering the tracing grossly inaccurate and unfit for analysis.
%
% Inputs:
% whiskerData -- structure input from .whiskers file
%
% tol -- tolerance for relative change in length allowed on adjacent frames
% (default 0.25)
%
...
for count = 1:length(whiskerData)
    ...
    whiskerData(count).whiskerLength = getWhiskerLength(whiskerData(count).x,whiskerData(count).y);
    
end

% Smooths over a 25-frame window.
whiskerLengthsSmoothed = smoothWhiskerData(whiskerData,12);

for count = 1:length(whiskerData)
    
    relMag = whiskerData(count).whiskerLength/whiskerLengthsSmoothed(count);
    
    if relMag > (1+tol) || relMag < 1/(1 + tol)
        whiskerData(count).contigFrame = 0;
    else
        whiskerData(count).contigFrame = 1;
    end
...
function smoothedWhiskerLengths = smoothWhiskerData(whiskerData,boxCarLength);
% function smoothedWhiskerLengths = smoothWhiskerData(whiskerData,boxCarLength);
%
% Smooths whisker data with a moving average filter.
%
% The smoothing window size = (2*boxCarLength + 1)
%
% John Sheppard, 3 November 2014

endTime = whiskerData(end).time;

whiskerLengths = nan(1,endTime);

% Assumes there are no duplicates.
whiskerIndexes = [whiskerData.time] + 1;

whiskerLengths(whiskerIndexes) = [whiskerData.whiskerLength];

whiskerLengthsSmoothed = nanmoving_average(whiskerLengths,boxCarLength);

for count = 1:numel(whiskerData)
    
    thisFrameIndex = whiskerData(count).time + 1; % +1 for matlab indexing

    smoothedWhiskerLengths(count) = whiskerLengthsSmoothed(thisFrameIndex);
